    ...
    for route in routes:
        ...
        for weekday in weekdays:
            for ampm in ampms.keys():
                ...
                grouped = rawdata.groupby(['route','weekday','am-pm']).get_group((route,weekday,ampm))[['datetime','avg-time']]
                ...
                # ts = df['avg-time'][:-6]
                # pred = df['avg-time'][-6:]
                # start, end = pred.index.tolist()[0], pred.index.tolist()[-1]
                ...
                acc_w += sum(abs(log_recover-ts)/ts)
                w += len(ts)
        acc += acc_w/w
    print ("mape: %f" %(acc/len(routes)))